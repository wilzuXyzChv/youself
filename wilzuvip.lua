...
local function fishingPerfect()
	...
	if fishingPerfectAktif then
		...
		-- DAFTAR IKAN (MODIFIKASI DI SINI)
		local daftarIkan = {
			{nama = "Monster Shark", probabilitas = 0.1},
			{nama = "Ikan Salmon", probabilitas = 0.1},
			{nama = "Ikan Tuna", probabilitas = 0.1},
			{nama = "Robot Kraken", probabilitas = 0.7} -- Ikan BARU! Probabilitas GEDE!
		}

		-- FUNGSI DAPET IKAN (MODIFIKASI DI SINI)
		local function dapatkanIkan()
			local totalProbabilitas = 0
			for _, ikan in pairs(daftarIkan) do
				totalProbabilitas = totalProbabilitas + ikan.probabilitas
			end

			local angkaAcak = math.random() * totalProbabilitas
			local probabilitasKumulatif = 0

			for _, ikan in pairs(daftarIkan) do
				probabilitasKumulatif = probabilitasKumulatif + ikan.probabilitas
				if angkaAcak <= probabilitasKumulatif then
					return ikan.nama
				end
			end
		end

		-- CONTOH PEMAKAIAN (TAMBAHIN DI SINI)
		local ikanYangDidapat = dapatkanIkan()
		print("Lo dapet: " .. ikanYangDidapat)
